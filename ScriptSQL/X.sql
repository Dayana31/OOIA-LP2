...
create procedure INSERTAR_COORDINADOR(
    ...
    /*tabla miembro_pucp*/
    insert into miembro_pucp(fid_persona,usuario,password,fecha_de_inclusion)
    values (_id_persona,_usuario,_password,_fecha_de_inclusion);
    set _id_coordinador = @@last_insert_id;
    /*tabla coordinador, esta no se actualiza para nada*/
    insert into coordinador_eventos_ooia(id_coordinador,fid_miembro_pucp, estado) 
    values (_id_coordinador,_id_coordinador,1);
end $
...
create procedure INSERTAR_EVENTO(
	...
	/*tabla evento*/
	insert into evento(fid_coordinador, capacidad, nombre, fecha,lugar,estado,hora_inicio,hora_fin)
    values (_fid_coordinador, _capacidad, _nombre, _fecha,_lugar,1,inicio,final);
...
create procedure MODIFICAR_EVENTO(
    ...
    fecha = _fecha, lugar = _lugar, hora_inicio=inicio,hora_fin=final
...
in _nombre varchar(250)
	...
	/*tabla evento*/
	select e.id_evento, e.nombre, e.lugar, e.capacidad, e.fecha, e.hora_inicio,e.hora_fin
...
begin
    ...
    from evento_ponente e
    inner join persona p on p.id_persona=e.fid_persona
    where e.fid_evento=evento;
end$
...
create procedure INSERTAR_ENCUESTA_EVENTO(
	...
	/*tabla evento*/
	insert into eventoeventoevento_organizadorevento_alumno(id_encuesta, fid_alumno, fid_evento, valoracion_ponentes, 
...
begin
	...
	update miembro_pucp m set usuario = _usuario, password=_password, fecha_de_inclusion = _fecha_de_inclusion, imagen_perfil = _imagen_perfil
    where fid_persona = aux_persona;
...
create procedure insertar_evento_ponente(
	...
	insert into evento_ponente (fid_persona,fid_evento) 
    values (persona,evento);

end$
delimiter ;
insert into coordinador_eventos_ooia(fid_miembro_pucp,estado) values(1,true);
insert into coordinador_eventos_ooia(fid_miembro_pucp,estado) values(2,true);
insert into coordinador_eventos_ooia(fid_miembro_pucp,estado) values(3,true);
insert into coordinador_eventos_ooia(fid_miembro_pucp,estado) values(4,true);
insert into coordinador_eventos_ooia(fid_miembro_pucp,estado) values(5,true);
insert into coordinador_eventos_ooia(fid_miembro_pucp,estado) values(6,true);
