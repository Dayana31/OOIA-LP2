...
create procedure INSERTAR_CITA(
	...
	insert into cita(fid_alumno, fid_asesor, fid_atencion, fid_horario, fecha, motivo, compromiso, asistio,activo) 
    	values(_fid_alumno, _fid_asesor, _fid_atencion, _fid_horario, _fecha, _motivo, _compromiso, false, true);
...
create procedure MODIFICAR_CITA(
	...
	update cita set fid_alumno = _fid_alumno, fid_asesor = _fid_asesor, fid_horario=_fid_horario, fid_atencion=_fid_atencion, fecha = _fecha,
    	motivo=_motivo, compromiso = _compromiso, asistio = _asistio
...
begin
	select mp.nombre as nombre_asesor, c.fecha, c.motivo,
	h.hora_inicio, h.hora_fin, ca.codigo, ca.descripcion,
	mp.nombre as asesor, c.asistio 
    	from cita c 
...
begin
	select mp.nombre as nombre_asesor, c.fecha, c.motivo, c.compromiso,
	h.hora_inicio, h.hora_fin, ca.codigo, ca.descripcion,
	mp.nombre as asesor, c.asistio 
    	from cita c 
	inner join horario h on c.fid_horario = h.id_horario
...
create procedure LISTAR_EVENTO(
	...
	select 	e.id_evento, e.nombre, e.descripcion, co.nombre as nombre_coordinador, c.nombre as nombre_categoria,
		e.capacidad, e.cupo, e.fecha, e.hora_inicio, e.hora_fin, e.lugar, e.imagen
    	from evento e
	inner join coordinador co on e.fid_coordinador = co.id_coordinador
